...
std::vector<std::pair<int, int>> clickHistory; // Stores the last three clicks

std::vector<std::pair<int, int>> clickHistorymod; // Stores the last three clicks 調整過後(因為會有偏差)
...
void fillTriangle(const std::vector<std::pair<int, int>>& points) {
    ...
    for (int x = min_x; x <= max_x; x++) {
        for (int y = min_y; y <= max_y; y++) {
            if (isPointInTriangle(x, y, points[0], points[1], points[2])) {
                setSquare(x, y, 2); // Set the grid square to green if it's inside the triangle
            }
        }
    }
}
...
void display() {
    ...
    // Draw the last three clicks as a triangle if available
    if (clickHistory.size() == 3) {
        //drawFilledTriangle(clickHistory);
